  ...
  return (
    <div className="slots-mgr">
      ...
      <p className="mgr-hint">
        Fill each seat from the dropdown or type a fresh name, then write that player’s
        <b> Start of match</b> and <b>Rest of match</b> orders right below — one instruction per line.
        Change a seat’s <b>Lane</b> to move it between teams, and set the teleport <b>order №</b> and
        <b> drop wave</b>. The <b>▾ More</b> button holds role label, objectives &amp; flag.
      </p>
      {LANE_IDS.map((L) => (
  ...
  return (
    <div className={"slot-editor" + (open ? " is-open" : "")}>
      <div className="se-head">
        ...
        <button className="btn-icon" title="More options — role label, objectives, flag" onClick={() => setOpen((o) => !o)}>{open ? "▲" : "▾"}</button>
      ...
      {/* Orders — always visible, the core of each seat */}
      <div className="se-orders-block">
        <div className="se-orders-grid">
          <Field label="Start of match" wide>
            <textarea
              key={"start-" + s.uid}
              className="mtextarea" rows="4"
              defaultValue={linesToText(s.start)}
              placeholder={"One order per line…\ne.g. Garrison the Obelisk\nJoin A3 rally"}
              onBlur={(e) => set({ start: textToLines(e.target.value) })}
            />
            <span className="se-orders-hint">{(s.start || []).length} {(s.start || []).length === 1 ? "order" : "orders"} · one per line</span>
          </Field>
          <Field label="Rest of match" wide>
            <textarea
              key={"rest-" + s.uid}
              className="mtextarea" rows="4"
              defaultValue={linesToText(s.rest)}
              placeholder={"One order per line…\ne.g. Hold Garrison\n3 marches to SA"}
              onBlur={(e) => set({ rest: textToLines(e.target.value) })}
            />
            <span className="se-orders-hint">{(s.rest || []).length} {(s.rest || []).length === 1 ? "order" : "orders"} · one per line</span>
          </Field>
        </div>
      </div>
